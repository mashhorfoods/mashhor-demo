...
import { mountOpsPortal, loadRegion, pageTitle, notFoundState, dataTable, opsStatusBadge, payBadge, dateTime, block, rows } from './shell.js';
...
function mountCustomerDetail({ root, id }) {
  return mountOpsPortal({ root, id: 'customers', head: 'page.ops.customerDetail', paint: async ({ main, can }) => {
    ...
    async function view() {
      const fresh = await opsData.customer(id);
      const nodes = [
      ...
      if (can('supervisor.manage') && can('attribution.view')) {
        ...
        const btn = el('button', { type: 'submit', class: 'c-btn c-btn--primary c-btn--sm' }, [el('span', { class: 'c-btn__label' }, t('ops.customers.reassign.action')), el('span', { class: 'c-btn__spinner', 'aria-hidden': 'true' })]);
        const form = el('form', { class: 'l-stack l-stack--8', onsubmit: async (e) => {
          e.preventDefault(); setButtonState(btn, 'loading');
          try { await opsData.reassignCustomer(id, supervisorId.value.trim() || null); toast({ title: t('ops.customers.reassign.done'), variant: 'success', duration: 3000 }); setButtonState(btn, 'success'); await refresh(); }
      ...
      if (can('customer.view')) nodes.push(block(t('ops.customers.bookings.title'), fresh.bookings.length
        ...
        : el('p', { class: 't-body-sm t-muted' }, t('ops.table.empty')), { id: 'ops-cus-bookings' }));
      ...
      if (can('payment.view')) nodes.push(block(t('ops.customers.payments.title'), fresh.payments.length
        ...
        : el('p', { class: 't-body-sm t-muted' }, t('ops.table.empty')), { id: 'ops-cus-payments' }));
      ...
      if (can('document.view')) nodes.push(block(t('ops.customers.documents.title'), fresh.documents.length
        ...
        : el('p', { class: 't-body-sm t-muted' }, t('ops.table.empty')), { id: 'ops-cus-documents' }));
      ...
      if (can('attribution.view')) nodes.push(block(t('ops.customers.attribution.title'), fresh.attributionHistory.length
        ...
        : el('p', { class: 't-body-sm t-muted' }, t('ops.table.empty')), { id: 'ops-cus-attribution' }));
