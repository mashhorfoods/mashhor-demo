...
function seed(customerId) {
  ...
  d.bookings = [
    ...
    booking('H1JED', { tripId: 'trip-dev-jed', service: 'hotels', status: 'pending', paymentStatus: 'unpaid', amount: 0, createdAt: iso(day(-29)), detail: { nights: 7, rooms: 2, checkin: dateOnly(21), checkout: dateOnly(28), noteAr: 'طلب فندق قيد التسعير من مختص نمبرون.', noteEn: 'Hotel request being priced by a Number One specialist.' } }),
  ...
  d.notifications = [
    ...
    note('3', { kind: 'booking', at: iso(day(-3, 16)), read: true, bookingId: 'NO-DEV-H1JED', titleAr: 'طلب الفندق مستلم', titleEn: 'Hotel request received', textAr: 'يعود إليك مختص نمبرون بخيارات الفندق والسعر.', textEn: 'A Number One specialist comes back with hotel options and a price.', href: 'account/bookings/?id=NO-DEV-H1JED' }),
...
export const DEV_CUSTOMER_ADAPTER = registerCustomerAdapter({
  ...
  async claimBooking(token, journey) {
    ...
    s.data.bookings.push({ id: b.reference, customerId: s.id, tripId, service: ctx.service, status: b.status === 'received' ? 'pending' : b.status, paymentStatus: paid ? 'paid' : 'unpaid', amount: b.total ?? 0, currency: b.currency ?? 'USD', supervisorId, ticketed: !!b.ticketed, createdAt: b.at ?? new Date().toISOString(), dev: true,
      detail: offer ? { route: offer.legs.map((l) => `${l.from.code} → ${l.to.code}`).join(' · '), dates: offer.legs.map((l) => l.departAt.slice(0, 10)), carrierAr: offer.carrier.nameAr, carrierEn: offer.carrier.nameEn, flights: offer.legs.map((l) => l.segments.map((sg) => sg.flightNumber).join(', ')).join(' / '), travellers } : { travellers, noteAr: 'طلب قيد المتابعة من مختص نمبرون.', noteEn: 'Request being handled by a Number One specialist.' } });
