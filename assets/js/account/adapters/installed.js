/* ============================================================================
   ...
   server (tests/contract-server.mjs), waiting for a deployed backend. No
   adapter is marked production until a real backend has been connected and
   this table says so. A production build with AUTH_PROVIDER=dev is refused
   by tools/write-env.mjs; a production build without a backend registers
   the not-connected adapters — it never serves development data.
   ========================================================================= */
...
if (wantsApi) {
  ...
  INSTALLED.push(
    { name: 'session-api', provider: 'backend session endpoints (hosted identity behind the backend)', environment: ENV.environment, status: 'implemented — not connected until a backend answers at API_BASE_URL', configSource: 'API_BASE_URL, AUTH_PUBLIC_CONFIG', capabilities: ['signUp', 'signIn', 'signOut', 'session', 'refresh', 'resetRequest', 'reset', 'changePassword'] },
    { name: 'api-customer', provider: 'customer backend API', environment: ENV.environment, status: 'implemented — not connected until a backend answers at API_BASE_URL', configSource: 'API_BASE_URL, DOCUMENT_SERVICE_CONFIG, PAYMENT_API_CONFIG, NOTIFICATION_CONFIG', capabilities: ['profile', 'trips', 'bookings', 'travellers', 'documents', 'documents.upload', 'documents.signedUrl', 'documents.delete', 'payments.paged', 'notifications', 'bookings.claim', 'legal.acceptance'] },
  );
...
if (legalReady) {
  ...
  INSTALLED.push({ name: ENV.legal.source === 'static' ? 'static-legal' : 'api-legal', provider: ENV.legal.source === 'static' ? 'static files supplied by the business' : 'customer backend API', environment: ENV.environment, status: 'implemented — connected when the documents exist at the configured source', configSource: 'LEGAL_DOCUMENT_CONFIG', capabilities: ['terms', 'privacy'] });
} else {
