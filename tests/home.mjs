...
for (const [w, h, tag] of [[390, 844, 'mobile'], [834, 1100, 'tablet'], [1440, 1000, 'desktop']]) {
  for (const loc of ['ar', 'en']) {
    ...
    ok(`${T} section order`, r.order.join(',') === 'hero-title,destinations-title,services-title,team-title,choose-title,offers-title,providers-title,newsletter-title,support-title', r.order.join(','));
