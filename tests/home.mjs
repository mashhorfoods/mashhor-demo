...
for (const [w, h, tag] of [[390, 844, 'mobile'], [834, 1100, 'tablet'], [1440, 1000, 'desktop']]) {
  for (const loc of ['ar', 'en']) {
    ...
    ok(`${T} section order`, r.order.join(',') === 'hero-title,services-title,why-title,choose-title,destinations-title,offers-title,journey-title,support-title,cta-title', r.order.join(','));
...
// ---------------------------------------------------------------- interaction (desktop, ar) + states
{
  ...
  ok('locale switch: English hero', /Travel with confidence/.test(r.h1), r.h1);
