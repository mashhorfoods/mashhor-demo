...
function sealCsp() {
  ...
  const STYLE_TOKEN = '__AUN_STYLE_HASHES__';
  if (!rules.includes(TOKEN)) {
    throw new Error('.htaccess no longer carries ' + TOKEN + ' — the CSP would ship without its hashes');
  }
  if (!rules.includes(STYLE_TOKEN)) {
    throw new Error('.htaccess no longer carries ' + STYLE_TOKEN + ' — style-src would ship without its hashes');
  }

  /* The policy is attached by <FilesMatch "\.html$">, so it governs EVERY html
     file at the document root, not index.html alone. 404.html carries its own
     stylesheet, and a hash list built from one page would leave the other
     page's <style> blocked — an error page with no styling, seen only by
     someone already lost. Gather from all of them. */
  const pages = fs.readdirSync(DIST)
  ...
  /* Every inline <script>, whatever its type. application/ld+json is included
     ...
     the search result it exists for. Read from the same page list as the
     styles below: one <FilesMatch> serves the policy to every html file here,
     so a script gathered from index.html alone would leave an inline script
     added to any other page refused with nothing to say why. */
  const hashes = [];
  const seen = new Set();
  for (const f of pages) {
    const body = fs.readFileSync(f, 'utf8');
    for (const m of body.matchAll(/<script(?![^>]*\bsrc=)([^>]*)>([\s\S]*?)<\/script>/gi)) {
  ...
  if (hashes.length === 0) {
    throw new Error('no page at the document root has an inline script — the CSP hashes would be empty');
  }

  /* style-src carried 'unsafe-inline' because the whole stylesheet is inline,
     and the note above this rule argued — correctly — that a style injection
     cannot execute where a script injection can. That is true, and it is not
     the whole of it: with 'unsafe-inline' an injected <style> can still
     reposition and repaint the page, which is how a fake form is laid over a
     real one. The stylesheet is generated by this build, so its bytes are
     known here, and a hash costs nothing that a wildcard was buying. */
  const styleHashes = [];
  const styleSeen = new Set();
  for (const f of pages) {
    const body = fs.readFileSync(f, 'utf8');
    for (const m of body.matchAll(/<style(?![^>]*\bsrc=)[^>]*>([\s\S]*?)<\/style>/gi)) {
      const h = 'sha256-' + crypto.createHash('sha256').update(m[1], 'utf8').digest('base64');
      if (styleSeen.has(h)) continue;
      styleSeen.add(h);
      styleHashes.push("'" + h + "'");
    }
  }
  if (styleHashes.length === 0) {
    throw new Error('no inline <style> found in any shipped page — style-src would ship empty');
  }
  /* A style attribute is not covered by a hash, so one appearing later would
     be blocked silently. There are none today; fail the build if that changes
     rather than shipping a page that renders wrong. */
  for (const f of pages) {
    const body = fs.readFileSync(f, 'utf8');
    if (/<[a-z][^>]*\sstyle=["']/i.test(body)) {
      throw new Error(path.basename(f) + ' has an inline style= attribute, which no hash can cover — '
        + 'move it into the stylesheet, or style-src needs \'unsafe-hashes\'');
    }
  }
  ...
  for (const f of pages) {
    const body = fs.readFileSync(f, 'utf8');
    for (const m of body.matchAll(/<script[^>]*\bsrc=["']([^"']+)["']/gi)) {
      const src = m[1];
      if (src.startsWith('/') || !/^https?:/i.test(src)) continue;
      if (!allowedHosts.some((h) => src.startsWith(h))) {
        throw new Error(path.basename(f) + ' loads a script from ' + src + ', which the CSP does not allow');
      }
    }
  }

  rules = rules.replace(TOKEN, hashes.join(' '));
  rules = rules.replace(STYLE_TOKEN, styleHashes.join(' '));
  fs.writeFileSync(htaccess, rules);
  console.log('  CSP                : ' + hashes.length + ' script + ' + styleHashes.length
    + ' style hashes sealed into .htaccess');
}
